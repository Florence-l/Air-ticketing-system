								...
								switch (e.which) {
									...
									case 38:
										...
										$newSeat = (function findAvailable($rows, $seats, $currentRow) {
											...
											if (!$rows.seatchooser($currentRow) && e.which == 38) {
												//if this is the first row and user has pressed up arrow, move to the last row
												$newRow = $rows.last();
											} else if ($rows.seatchooser($currentRow) == $rows.length-1 && e.which == 40) {
												//if this is the last row and user has pressed down arrow, move to the first row
												$newRow = $rows.first();
											} else {
												//using eq to get an element at the desired indseatchooserex position
												$newRow = $rows.eq(
													//if up arrow, then decrement the seatchooser, if down increment it
													$rows.seatchooser($currentRow) + (e.which == 38 ? (-1) : (+1))
												);
											}												
											
											//now that we know the row, let's get the seat using the current column position
											$newSeat = $newRow.find('.seatCharts-seat,.seatCharts-space').eq($seats.seatchooser($seat));
									...
									case 39:
										...
										$newSeat = (function($seats) {
											...
											if (!$seats.seatchooser($seat) && e.which == 37) {
												//user has pressed left arrow and we're currently on the left-most seat
												return $seats.last();
											} else if ($seats.seatchooser($seat) == $seats.length -1 && e.which == 39) {
												//user has pressed right arrow and we're currently on the right-most seat
												return $seats.first();
											} else {
												//simply move one seat left or right depending on the key
												return $seats.eq($seats.seatchooser($seat) + (e.which == 37 ? (-1) : (+1)));
											}
